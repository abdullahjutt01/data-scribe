...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function renderActivity(report) {
    ...
    summaryContainer.innerHTML = `
      ...
      <div>🔍 <strong>Layer 1 (Social):</strong> ${report.layers?.social || 'N/A'}</div>
      <div>🕷️ <strong>Layer 2 (Website):</strong> ${report.layers?.website || 'N/A'}</div>
      <div>🔑 <strong>Layer 3 (API):</strong> ${report.layers?.api || 'N/A'}</div>
    `;
  }
  ...
  async function exportGoogleSheets() {
    if (!currentReport) { showDashToast('❌ No report loaded yet'); return; }

    // 1. Get saved Script URL from storage
    const keyRes = await chrome.runtime.sendMessage({ action: 'getApiKeys' });
    const scriptUrl = keyRes?.keys?.SCRIPT_URL;
    if (!scriptUrl || !scriptUrl.startsWith('https://script.google.com')) {
      showDashToast('⚠️ No Script URL set. Go to Settings → Google Sheets Web App URL');
      return;
    }

    showDashToast('📤 Sending to Google Sheets...');
    const report = currentReport;

    // 2. Build complete flat payload — every field mapped to a column
    const payload = {
      companyName:   report.companyName  || '',
      websiteUrl:    report.websiteUrl   || '',
      industry:      report.official?.industry      || '',
      headquarters:  report.official?.headquarters  || '',
      yearFounded:   report.official?.yearFounded   || '',
      employeeCount: report.official?.employeeCount || '',
      revenue:       report.official?.revenue       || '',

      // All emails as comma-separated string
      emails: (report.directContact?.emails || []).join(', '),

      // All phones as comma-separated string
      phones: (report.directContact?.phones || []).join(', '),

      // Social Platforms
      linkedin:  report.socialLinks?.LinkedIn?.url  || report.socialLinks?.['LinkedIn Personal']?.url || '',
      facebook:  report.socialLinks?.Facebook?.url  || '',
      instagram: report.socialLinks?.Instagram?.url || '',
      twitter:   report.socialLinks?.['Twitter/X']?.url || report.socialLinks?.X?.url || '',
      youtube:   report.socialLinks?.YouTube?.url   || '',
      tiktok:    report.socialLinks?.TikTok?.url    || '',

      // People: "Name (Title) <email>" format
      people: (report.people || []).map(p => {
        let str = p.name || '';
        if (p.title) str += ` (${p.title})`;
        if (p.email) str += ` <${p.email}>`;
        if (p.phone) str += ` 📞${p.phone}`;
        return str;
      }).join(' | '),

      // Tech stack
      techStack: (report.activity?.techStack || []).join(', '),

      timestamp: new Date(report.timestamp || Date.now()).toLocaleString(),
    };

    try {
      // Direct POST to Apps Script (no-cors as required by Google)
      ...
      showDashToast(`✅ "${report.companyName}" exported to Google Sheets!`);

      // Also copy TSV row to clipboard for backup paste
      const tsvRow = [
        payload.companyName, payload.websiteUrl, payload.emails,
        payload.phones, payload.linkedin, payload.facebook,
        payload.instagram, payload.people, payload.industry, payload.timestamp
      ].map(v => String(v).replace(/\t/g, ' ').replace(/\n/g, ' ')).join('\t');
      await navigator.clipboard.writeText(tsvRow).catch(() => {});

    } catch (err) {
      showDashToast(`❌ Export failed: ${err.message}`);
    }
  }
